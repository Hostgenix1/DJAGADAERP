    ...
    <div class="col-lg-8">
        <div class="card card-primary card-outline">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-university mr-1"></i> Edit Bank Account</h3>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('bank-accounts.update', $bankAccount->id) }}">
                            ...
                            <div class="form-group">
                                ...
                                <select name="currency_id" id="currency_id"
                                    ...
                                    @foreach($currencies as $id => $code)
                                        <option value="{{ $id }}" {{ old('currency_id', $bankAccount->currency_id) == $id ? 'selected' : '' }}>{{ $code }}</option>
                                    @endforeach
                                </select>
                                @error('currency_id')
                                    <span class="text-danger">{{ $message }}</span>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="notes" id="notes" rows="3"
                                  class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $bankAccount->notes) }}</textarea>
                        @error('notes')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
