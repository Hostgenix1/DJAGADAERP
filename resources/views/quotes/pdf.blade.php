...
<tr>
    ...
    <td class="col-price">{{ $quote->currency?->code == 'AED' ? 'AED' : ($quote->currency?->symbol ?? 'AED') }} {{ number_format($item->unit_price, 2) }}</td>
    <td class="col-total">{{ $quote->currency?->code == 'AED' ? 'AED' : ($quote->currency?->symbol ?? 'AED') }} {{ number_format($item->line_total, 2) }}</td>
</tr>
@empty
