    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="card card-primary card-outline">
                <div class="card-header"><h3 class="card-title">Edit {{ $quote->number }}</h3></div>
                <div class="card-body">
                    <div class="form-row">
                    ...
                    <table class="table table-sm" id="items-table">
                        <thead><tr><th style="width:30%">Description</th><th>Qty</th><th>Unit</th><th>Price</th><th>Tax %</th><th>Disc %</th><th>Line Total</th><th></th></tr></thead>
                        <tbody></tbody>
                        <tfoot>
                    ...
                    <div class="form-group"><label>Notes</label><textarea name="notes" class="form-control" rows="2">{{ old('notes', $quote->notes) }}</textarea></div>
                    <div class="form-group"><label>Terms</label><textarea name="terms" class="form-control" rows="2">{{ old('terms', $quote->terms) }}</textarea></div>
                </div>
            </div>
        </div>
        <div class="col-lg-4">
            <div class="card"><div class="card-body">
                <button type="submit" class="btn btn-primary btn-block"><i class="fas fa-save"></i> Update Quote</button>
                <a href="{{ route('quotes.show', $quote) }}" class="btn btn-default btn-block">Cancel</a>
            </div></div>
        </div>
    </div>
</form>
...
$itemsJson = $quote->items->map(fn($i) => ['product_id'=>$i->product_id,'description'=>$i->description,'qty'=>$i->qty,'unit'=>$i->unit,'price'=>$i->unit_price,'tax'=>$i->tax_rate,'disc'=>$i->discount_pct])->values();
@endphp
@endsection
...
$(function () {
    ...
    $(document).on('change', '.prod-select', function() {
        ...
        if(opt.text() !== 'Manual') $row.find('input[name$="[description]"]').val(opt.text());
        recalc();
    });
