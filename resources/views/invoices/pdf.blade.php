...
<tr>
    ...
    <td class="col-price">{{ $invoice->currency?->code == 'AED' ? 'AED' : ($invoice->currency?->symbol ?? 'AED') }} {{ number_format($item->unit_price, 2) }}</td>
    <td class="col-tax">{{ $item->tax_rate > 0 ? $item->tax_rate.'%' : '-' }}</td>
    <td class="col-total">{{ $invoice->currency?->code == 'AED' ? 'AED' : ($invoice->currency?->symbol ?? 'AED') }} {{ number_format($item->line_total, 2) }}</td>
</tr>
@empty
