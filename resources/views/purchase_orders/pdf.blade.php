...
<table class="meta-table">
    <tr>
        ...
        <td class="meta-box" style="width:33%; border-left:none;">
            ...
            <div class="meta-row"><span class="label">PO Date:</span> <span class="value">{{ $po->po_date?->format('d/m/Y') }}</span></div>
            @if($po->expected_delivery)<div class="meta-row"><span class="label">Expected Delivery:</span> <span class="value">{{ $po->expected_delivery->format('d/m/Y') }}</span></div>@endif
            @if($po->reference_no)<div class="meta-row"><span class="label">Reference No:</span> <span class="value">{{ $po->reference_no }}</span></div>@endif
            @if($po->payment_terms)<div class="meta-row"><span class="label">Payment Terms:</span> <span class="value">{{ $po->payment_terms }}</span></div>@endif
            @if($po->delivery_terms)<div class="meta-row"><span class="label">Delivery Terms:</span> <span class="value">{{ $po->delivery_terms }}</span></div>@endif
            @if($po->port_of_loading)<div class="meta-row"><span class="label">Port of Loading:</span> <span class="value">{{ $po->port_of_loading }}</span></div>@endif
            @if($po->port_of_discharge)<div class="meta-row"><span class="label">Port of Discharge:</span> <span class="value">{{ $po->port_of_discharge }}</span></div>@endif
            @if($po->goods_origin)<div class="meta-row"><span class="label">Goods Origin:</span> <span class="value">{{ $po->goods_origin }}</span></div>@endif
        </td>
...
<table class="items">
    ...
    @foreach($po->items as $i => $item)
        @php
            $effTax = $item->tax_rate ?? $po->vat_rate;
            $effTax = ($item->tax_rate === null && $po->vat_mode === 'none') ? 0 : $effTax;
            $effTax = $effTax ?? 0;
        @endphp
        <tr>
            <td>{{ $i + 1 }}</td>
            ...
            <td class="num">{{ $po->currency?->code }} {{ \App\Support\SettingsHelper::formatMoney($item->unit_price) }}@if($item->unit) / {{ $item->unit }}@endif</td>
            <td class="num">{{ $effTax > 0 ? rtrim(rtrim($effTax,'0'),'.').'%' : '0%' }}</td>
            <td class="num">{{ $po->currency?->code }} {{ \App\Support\SettingsHelper::formatMoney($item->line_total * (1 + $effTax/100)) }}</td>
        </tr>
    @endforeach
    </tbody>
</table>
...
<table class="totals">
    <tr><td>Total Amount</td><td>{{ $po->currency?->code }} {{ \App\Support\SettingsHelper::formatMoney($po->subtotal + $po->tax_amount) }}</td></tr>
    <tr><td>Tax ({{ $po->vat_mode === 'included' ? 'Incl.' : ($po->vat_rate ? rtrim(rtrim($po->vat_rate,'0'),'.').'%' : '0%') }})</td><td>{{ $po->currency?->code }} {{ \App\Support\SettingsHelper::formatMoney($po->tax_amount) }}</td></tr>
    @if((float)$po->discount > 0)
    <tr><td>Discount</td><td>- {{ $po->currency?->code }} {{ \App\Support\SettingsHelper::formatMoney($po->discount) }}</td></tr>
    @endif
...
<div class="terms-box">
    ...
    <div class="terms-text">{{ $po->notes ?: $company['footer'] }}</div>
</div>
@endif
